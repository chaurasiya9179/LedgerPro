...
import { Home, Mail, Lock, X, User, Hash } from 'lucide-react';
...
export default function AuthScreen({ setSession, showAlert, onBack }) {
  ...
  const handleAuth = async (e) => {
    ...
    // 🔥 1. Strict Validation Logic (Sirf Name aur Pincode check hoga) 🔥
    if (!isLogin) {
       if (!fullName.trim() || !pincode.trim()) { 
           setError("Sign up ke liye Naam aur Pincode zaroori hai."); 
           setLoading(false); 
           return; 
       }
       if (!/^\d{6}$/.test(pincode.trim())) {
           setError("Pincode bilkul 6 digit ka hona chahiye."); 
           setLoading(false); 
           return; 
       }
    }
    ...
    try {
      ...
      if (data.access_token) {
         ...
         // 🔥 Profile me sirf Name aur Pincode save hoga, kyc_status 'Pending' rahega 🔥
         if (!existingProfile || existingProfile.length === 0) {
            await fetch(`${supabaseUrl}/rest/v1/profiles`, {
               ...
               body: JSON.stringify({
                  ...
                  admin_id: userMeta.role === 'admin' ? data.user.id : null,
                  full_name: userMeta.full_name || fullName.trim(),
                  pincode: userMeta.pincode || pincode.trim(),
                  kyc_status: 'Pending', // 👈 Yeh tag andar jate hi dashboard lock kar dega
                  createdAt: Date.now()
               })
            });
         }
         ...
         setIsLogin(true);
         setPassword(''); setFullName(''); setPincode('');
      }
          ...
          {!isLogin && (
             <div className="space-y-5 animate-in fade-in zoom-in duration-300">
                ...
                <div className="space-y-2 relative">
                  ...
                  <input type="text" maxLength="6" placeholder={t("Pincode (6 Digits)", "Area Pincode (6 Ank)")} value={pincode} onChange={(e) => setPincode(e.target.value.replace(/\D/g, ''))} className="w-full pl-12 pr-5 py-4 bg-black/40 border border-white/10 rounded-2xl text-white focus:ring-2 focus:ring-cyan-500/50 focus:border-cyan-500 transition-all outline-none font-mono" required={!isLogin} />
                </div>
             </div>
          )}
